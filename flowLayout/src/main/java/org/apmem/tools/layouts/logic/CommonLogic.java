...
public class CommonLogic {
    ...
    public static void applyGravityToLines(List<LineDefinition> lines, int realControlLength, int realControlThickness, ConfigDefinition config) {
        ...
        for (int i = 0; i < linesCount; i++) {
            ...
            // ugly, but useful. 用于在最后一行不均分父控件宽度
            if(gravity == Gravity.FILL && i == linesCount - 1) {
    ...
    public static void applyGravityToLines(List<LineDefinition> lines, int realControlLength, int realControlThickness, ConfigDefinition config, ConfigDefinition firstLineConfig) {
        final int linesCount = lines.size();
        if (linesCount <= 0) {
            return;
        }
        ...
        for (int i = 0; i < linesCount; i++) {
            ...
            container.left = 0;
            if(i == 0) {
                container.right = firstLineConfig.getMaxLength();
            } else {
                container.right = realControlLength;
            }
            container.bottom = childThickness + extraThickness + excessOffset;
            ...
            if(i == 0) {
                applyGravityToLine(child, firstLineConfig);
            } else {
                applyGravityToLine(child, config);
            }
    ...
    public static void fillLines(List<ViewDefinition> views, List<LineDefinition> lines, ConfigDefinition config, ConfigDefinition firstLineConfig) {
        LineDefinition currentLine = new LineDefinition(firstLineConfig);
        lines.add(currentLine);
        final int count = views.size();
        for (int i = 0; i < count; i++) {
            final ViewDefinition child = views.get(i);
